...
Rect RenderSystem::getScreenViewPort(const Camera& aCamera) const
{
	...
	if ((mWindow->getSize().x / mAspectRatio.x) < (mWindow->getSize().y / mAspectRatio.y))
	{
		...
		// std::cout << "ScreenViewPort.x " << screenViewPort.x << std::endl;
		// std::cout << "ScreenViewPort.y " << screenViewPort.y << std::endl;
		// std::cout << "ScreenViewPort.h " << screenViewPort.h << std::endl;
		// std::cout << "ScreenViewPort.w " << screenViewPort.w << std::endl;
	}
	else if ((mWindow->getSize().x / mAspectRatio.x) > (mWindow->getSize().y / mAspectRatio.y))
	{
		// Pillarbox
...
void RenderSystem::render(const Scene& aScene) const
{
	...
	for (Camera* camera : cameras)
	{
		...
		// screenViewPort.y -= screenViewPort.h / 2;
		// screenViewPort.h *= 2;
...
void RenderSystem::setBackgroundColor(const Color& aBackgroundColor) { mBackgroundColor = aBackgroundColor; }

Color RenderSystem::getBackgroundColor() const { return mBackgroundColor; }
