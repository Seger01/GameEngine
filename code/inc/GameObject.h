 ...
 *
 * GameObjects are the basic building blocks of the game. A GameObject is essentially
 * a container for components, which have a shared Transform. A GameObject always possesses a Transform, because it is
 * necessary to have a position in the game world. The GameObject class has various methods for manipulating its
 * components. Everything that is visible or interactable in the game is a GameObject with components attached to it.
 * Components can be added to the GameObjects at runtime, from the Behaviourscripts. All GameObjects are stored in
 * scenes, which can be seen as the different levels of a game.
 */
class GameObject
{
public:
