...
TEST(SaveGameManager, createGetDelete)
{
	SaveGameManager& saveGameManager = EngineBravo::getInstance().getSaveGameManager();
	SaveGame& saveGame = saveGameManager.createSaveGame("save1", "saves/save1.json");
	ASSERT_EQ(&saveGame, &saveGameManager.getSaveGame("save1"));
	saveGameManager.deleteSaveGame("save1");
	ASSERT_THROW(saveGameManager.getSaveGame("save1"), std::invalid_argument);
}

TEST(SaveGameManger, invalidGet)
{
	SaveGameManager& saveGameManager = EngineBravo::getInstance().getSaveGameManager();
	// Make sure it doesn't exist from previous tests
	try
	{
		saveGameManager.deleteSaveGame("save1");
	}
	catch (std::exception& e) // Ignore if it throws
	{
	}
	ASSERT_THROW(saveGameManager.getSaveGame("save1"), std::invalid_argument);
}

TEST(saveGameManager, invalidDelete)
{
	SaveGameManager& saveGameManager = EngineBravo::getInstance().getSaveGameManager();
	ASSERT_THROW(saveGameManager.deleteSaveGame("save1"), std::logic_error);
}

TEST(SaveGameManager, deleteAndGet)
{
	SaveGameManager& saveGameManager = EngineBravo::getInstance().getSaveGameManager();
	SaveGame& saveGame = saveGameManager.createSaveGame("save1", "saves/save1.json");
	saveGame.store();
	saveGameManager.deleteSaveGame("save1", true);
	ASSERT_THROW(saveGameManager.getSaveGame("save1"), std::invalid_argument);
}